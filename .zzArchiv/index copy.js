...
class ConfigUpdater {
    ...
    DEFAULT_MANDATORY = "$mandatory" // "$$" is used in test/config-updater.test.js
    DEFAULT_IF = "$if" // "$$" is used in test/config-updater.test.js
    DEFAULT_IF_VALUE = "$if_value" // "$$" is used in test/config-updater.test.js
    ...
    /**
     * Retrieve the value of a property with the help of repositories (environment,...)
     *
     ...
     * @see updateConfig
     */
    getCmValue(cm_or_any, propertyName, parentPropertyName) {
     ...
     * @param {string}       initialParentPropertyName=""  Optional name of the parent property. Used to add the branch-name for $$: "?..." replacements. This is *only* necessary if the function is called with a partial branch.
     * @returns {Object|Array} Returns the given `config`-parameter-object
     */
    updateConfig(config, exclude = [], initialParentPropertyName = "") {
                ...
                if (
                    ...
                    if (result && typeof result == "object") {
                        ...
                        if (isArray) {
                            ...
                            traverseConfig(result, indexKey, parentPropertyName)
                        }
                    }
                }
            }
...
function toCm(
    ...
    // if & value => value = {CM|any} if = function(value);
    // mandatory => throw an error if result is undefined
) {
